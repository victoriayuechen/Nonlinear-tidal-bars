...
function APVM_run(order,degree,D₀,u₀,topography,forcefunc,dir,periodic::Bool,Tend,dt,model,DC)
    ...
    mu(x,t::Real) = VectorValue(0.1*sin(D*x[2])*cos(E*(x[1]-xref))*sin(C*t)+1.0,
    0.1*sin(A*x[1])*sin(B*x[2])*sin(C*t)+1.0)
    mu(t::Real) = x -> mu(x,t)
...
APVM_run(0,4,D₀,u₀,topography,forcfunc,joinpath(outputdir,dir),true,Tend,dt,model,DC)
