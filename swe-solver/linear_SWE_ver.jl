...
function linear_SWE(order,degree, dt, tstamp)
    ...
    #Load up the model
    model = GmshDiscreteModel("swe-solver/meshes/1x4periodic01.msh")
...
if isdir(dir)
    output_file = paraview_collection(joinpath(dir,"linear$tstamp"))do pvd
...
#linear_SWE(order, degree, dt, stamp)
...
for t in time_steps
    ...
    eul2, ehl2 = linear_SWE(order, degree, t, stamp)
...
savefig(p3, joinpath(dir_plots,"time_conv.pdf"))
